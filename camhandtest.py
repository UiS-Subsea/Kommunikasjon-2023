...
class gstreamerPipe(Thread):
    ...
    def createPipe(self):
        if self.pipeId == "stereo1" or self.pipeId == "stereo2":
            ...
            gstStr = (f"v4l2src device=/dev/video={self.sourceID} ! video/x-raw(memory:NVMM), width=1920, heigth=1080, framerate=30/1, format=NV12 ! nvv4l2h264enc insert-sps-pps=true bitrate={self.bitrate} ! rtph264pay ! udpsink host={self.multicastGroup} port={self.port} auto-multicast=true")
